...
from data_structures import (
    PowerUpType, EnemyAIType, ParticleType, FinisherPhase,
    ShipState, Asteroid, Bullet, Enemy, PowerUp, Particle, FloatingText
)
...
def get_ship_points(x: float, y: float, angle: float, scale_mult: float = 1.0) -> List[Tuple[float, float]]:
...
def calculate_pulse(time: float, frequency: float = 0.1, amplitude: float = 0.5, offset: float = 0.5) -> float:
...
class TextCache:
    ...
    def get_text(self, text: str, font: pygame.font.Font, color: Tuple[int, int, int]) -> pygame.Surface:
    ...
    def update(self) -> None:
        ...
        if self.frame_count % 300 == 0:
            old_keys = [key for key, (_, last_access) in self.cache.items() 
...
def load_game_state() -> bool:
    ...
    try:
        ...
        print(f"[load_game_state] Loaded: High score {g_game_state['high_score']}, "
...
ACHIEVEMENT_CONDITIONS = {
    ...
    'speed_demon': lambda: g_game_state['upgrade_levels']['max_speed'] >= Cfg.achievement_speed_demon_level,
    'crystal_hoarder': lambda: g_game_state['lifetime_crystals'] >= Cfg.achievement_crystal_hoarder_amount
}
...
def check_achievement(achievement_id: str) -> bool:
    ...
    if achievement_id in ACHIEVEMENT_CONDITIONS and ACHIEVEMENT_CONDITIONS[achievement_id]():
        ...
        create_floating_text(g_screen_width // 2, g_screen_height // 2 - 100,
                           f"ACHIEVEMENT: {achievement['name']}", Cfg.colors['gold'])
        create_floating_text(g_screen_width // 2, g_screen_height // 2 - 70,
                           f"+{achievement['reward']} Crystals", Cfg.colors['crystal'])
        save_game_state()
        return True
...
def get_damage_multiplier() -> float:
    ...
    Globals:
        Reads g_game_state
    """
    return 1.0 + (g_game_state['upgrade_levels'].get('damage', 0) * Cfg.upgrades['damage']['multiplier_per_level'])
...
def get_fire_rate_multiplier() -> float:
    ...
    Globals:
        Reads g_game_state
    """
    return 1.0 - (g_game_state['upgrade_levels'].get('fire_rate', 0) * Cfg.upgrades['fire_rate']['reduction_per_level'])
...
def get_speed_multiplier() -> float:
    ...
    Globals:
        Reads g_game_state
    """
    return 1.0 + (g_game_state['upgrade_levels'].get('max_speed', 0) * Cfg.upgrades['max_speed']['multiplier_per_level'])
...
def get_dash_cooldown_reduction() -> float:
    ...
    Globals:
        Reads g_game_state
    """
    return g_game_state['upgrade_levels'].get('dash_cooldown', 0) * Cfg.upgrades['dash_cooldown']['reduction_per_level']
...
def create_asteroid(x: Optional[float] = None, y: Optional[float] = None, 
                   size: int = 3, is_boss: bool = False, has_crystals: bool = False) -> Asteroid:
    ...
    asteroid = Asteroid(
        ...
        shape=[random.randint(Cfg.asteroid_shape_variance_min, Cfg.asteroid_shape_variance_max) 
...
def create_enemy(x: Optional[float] = None, y: Optional[float] = None) -> Enemy:
    ...
    return Enemy(
        x=x, y=y,
        angle=random.uniform(0, 360),
        fire_cooldown=Cfg.enemy_fire_rate + random.randint(-Cfg.enemy_fire_rate_variance, Cfg.enemy_fire_rate_variance),
...
def create_powerup(x: float, y: float, force_type: Optional[PowerUpType] = None) -> None:
    ...
    """
    if force_type == PowerUpType.CRYSTAL or (force_type is None and random.random() < Cfg.powerup_crystal_chance):
        powerup_type = PowerUpType.CRYSTAL
    elif force_type:
        powerup_type = force_type
    elif random.random() < Cfg.powerup_drop_chance:
        powerup_type = random.choice([PowerUpType.RAPID, PowerUpType.TRIPLE, PowerUpType.SHIELD, PowerUpType.LIFE])
    else:
        return
    ...
    lifetime = int(Cfg.powerup_lifetime * (1 + (area_multiplier - 1) * Cfg.powerup_area_scaling_factor))
...
def create_explosion(x: float, y: float, count: int = Cfg.particle_explosion_count, 
                    color_base: Tuple[int, int, int] = (255, 100, 0), is_enemy: bool = False) -> None:
    ...
    g_game_state['effects']['screen_shake'] = min(g_game_state['effects']['screen_shake'] + shake_amount, 
    ...
    area_factor = min(2.0, math.sqrt((g_screen_width * g_screen_height) / (Cfg.screen_width * Cfg.screen_height)))
...
def create_dash_trail() -> None:
    ...
    for _ in range(Cfg.dash_trail_particle_count):
        offset = random.uniform(-Cfg.dash_trail_offset_range, Cfg.dash_trail_offset_range) * g_scale_factor
...
def create_powerup_streak(powerup_x: float, powerup_y: float, target_x: float, 
                         target_y: float, color: Tuple[int, int, int]) -> None:
    ...
    create_particles(powerup_x, powerup_y, {
        'count': burst_count,
        'color': color,
        'speed_range': (1, 3),
        'life_base': 25,
...
def create_thruster_particles() -> None:
    ...
    for _ in range(Cfg.particle_thruster_count):
        particle = g_particle_pool.get()
        if particle:
            particle.x = base_x + random.randint(-Cfg.particle_thruster_spread, Cfg.particle_thruster_spread)
            particle.y = base_y + random.randint(-Cfg.particle_thruster_spread, Cfg.particle_thruster_spread)
            particle.vx = -cos_a * Cfg.particle_thruster_speed * g_scale_factor + random.uniform(-Cfg.particle_thruster_velocity_spread, Cfg.particle_thruster_velocity_spread)
            particle.vy = -sin_a * Cfg.particle_thruster_speed * g_scale_factor + random.uniform(-Cfg.particle_thruster_velocity_spread, Cfg.particle_thruster_velocity_spread)
            particle.life = 20
...
def create_respawn_particles() -> None:
    ...
    for _ in range(Cfg.particle_respawn_rate):
        angle = random.uniform(0, 360)
        distance = random.uniform(Cfg.particle_respawn_spiral_min, Cfg.particle_respawn_spiral_max) * g_scale_factor
        sin_a, cos_a = get_sin_cos(angle)
        ...
        if random.random() < Cfg.particle_respawn_center_chance:
            particle = g_particle_pool.get()
            if particle:
                particle.x = g_ship.x + random.uniform(-Cfg.particle_respawn_center_spread, Cfg.particle_respawn_center_spread) * g_scale_factor
                particle.y = g_ship.y + random.uniform(-Cfg.particle_respawn_center_spread, Cfg.particle_respawn_center_spread) * g_scale_factor
...
def shoot_bullet(is_enemy: bool = False, enemy_x: float = 0.0, enemy_y: float = 0.0, 
                enemy_angle: float = 0.0) -> None:
    ...
    if is_enemy:
        ...
        particle_count = Cfg.particle_muzzle_flash_triple if g_ship.triple_shot > 0 else Cfg.particle_muzzle_flash_base
...
def add_combo() -> None:
    ...
    if combo['current'] >= Cfg.combo_text_threshold:
        create_floating_text(g_ship.x, g_ship.y - 30, f"COMBO x{combo['current']}!", Cfg.colors['gold'])
        
        if combo['kills'] % Cfg.combo_pulse_interval == 0 and combo['current'] >= Cfg.combo_text_threshold:
            combo['pulse'] = min(Cfg.combo_max_pulse, combo['current'] * 2)
            g_game_state['effects']['screen_shake'] = min(g_game_state['effects']['screen_shake'] + 8, 
                                                       scaled(Cfg.max_screen_shake))
            ...
            g_game_state['effects']['screen_shake'] = min(g_game_state['effects']['screen_shake'] + 5, 
                                                       scaled(Cfg.max_screen_shake))
...
def start_finisher_execution(target: Enemy) -> None:
    ...
    total_finisher_frames = (Cfg.finisher_lock_on_time + Cfg.finisher_pre_impact_time + 
                           Cfg.finisher_impact_time + Cfg.finisher_post_impact_time)
...
def update_finisher() -> None:
    ...
    if finisher_state['execution_timer'] <= 0:
        if finisher_state['phase'] == FinisherPhase.LOCK_ON:
            ...
            total_finisher_frames = (Cfg.finisher_pre_impact_time + Cfg.finisher_impact_time + 
            ...
            finisher_state['shockwave_radius'] = 10 * g_scale_factor + (max_radius - 10 * g_scale_factor) * progress
            
            # Apply damage at specific points
            damage_points = [0.3, 0.6]
            for dmg_point in damage_points:
                if dmg_point <= progress < dmg_point + 0.02:
                    apply_shockwave_damage(finisher_state['impact_x'], finisher_state['impact_y'], 
    ...
    if finisher_state['phase'] == FinisherPhase.LOCK_ON:
        finisher_state['lock_on_progress'] = 1.0 - (finisher_state['execution_timer'] / Cfg.finisher_lock_on_time)
...
def apply_shockwave_damage(x: float, y: float, radius: float) -> None:
    ...
    for enemy in g_enemies[:]:
        ...
        if dist_sq < radius * radius:
            ...
            damage = Cfg.finisher_damage_close if dist < radius * Cfg.finisher_shockwave_close_range else Cfg.finisher_damage_far
            enemy.health -= damage
            enemy.hit_flash = Cfg.asteroid_hit_flash_duration
            
            if dist > 0:
                knockback_force = (1 - dist / radius) * Cfg.finisher_knockback_force * g_scale_factor
                dx = enemy.x - x
                dy = enemy.y - y
...
def handle_bullet_collisions() -> None:
    ...
    for i, bullet in enumerate(g_bullets):
        ...
        for j, asteroid in enumerate(g_asteroids):
            ...
            if check_collision(bullet, asteroid, scaled(Cfg.bullet_radius), asteroid.radius + margin):
...
def handle_ship_damage() -> bool:
    ...
    if g_ship.shield_active > 0:
        ...
        g_game_state['effects']['screen_shake'] = min(g_game_state['effects']['screen_shake'] + 10, 
                                                   scaled(Cfg.max_screen_shake))
...
def handle_powerup_collisions() -> None:
    ...
    for i, powerup in enumerate(g_powerups):
        if check_collision(g_ship, powerup, scaled(Cfg.powerup_pickup_radius), scaled(Cfg.powerup_visual_radius)):
...
def apply_powerup(powerup_type: PowerUpType) -> None:
    ...
    g_game_state['effects']['screen_shake'] = min(g_game_state['effects']['screen_shake'] + 3, 
                                               scaled(Cfg.max_screen_shake))
...
def update_ship_timers() -> None:
    ...
    g_game_state['effects']['aura_rotation'] = (g_game_state['effects']['aura_rotation'] + 
                                             Cfg.powerup_aura_rotation_speed * g_game_state['time_scale']) % 360
...
def update_enemy_shooting(enemy: Enemy) -> None:
    ...
    if enemy.fire_cooldown <= 0:
        ...
        if min_fire_distance < distance < max_fire_distance:
            aim_angle = enemy.angle + random.uniform(-Cfg.enemy_aim_inaccuracy, Cfg.enemy_aim_inaccuracy)
            shoot_bullet(is_enemy=True, enemy_x=enemy.x, enemy_y=enemy.y, enemy_angle=aim_angle)
            enemy.fire_cooldown = Cfg.enemy_fire_rate + random.randint(-Cfg.enemy_fire_rate_variance, 
                                                                       Cfg.enemy_fire_rate_variance)
...
def update_bullets() -> None:
    ...
    g_bullets = [b for b in g_bullets if b.life > 0 and 
               0 <= b.x <= g_screen_width and 0 <= b.y <= g_screen_height]
...
def update_enemy_bullets() -> None:
    ...
    g_enemy_bullets = [b for b in g_enemy_bullets if b.life > 0 and 
                    0 <= b.x <= g_screen_width and 0 <= b.y <= g_screen_height]
...
def update_finisher_meter() -> None:
    ...
    if g_game_state['combo']['current'] == 0 and finisher['meter'] > 0:
        ...
        finisher['meter'] = max(0.0, finisher['meter'] - decay_per_frame * g_game_state['time_scale'])
        if finisher['meter'] < 100.0:
            finisher['ready'] = False

def create_floating_text(x: float, y: float, text: str, color: Optional[Tuple[int, int, int]] = None) -> None:
    ...
    g_floating_texts.append(FloatingText(
        x=x + random.randint(-Cfg.floating_text_spread, Cfg.floating_text_spread) * g_scale_factor,
        y=y,
        text=text,
        color=color,
...
def get_polygon_points(obj: Any, num_points: int, base_radius: float, 
...
def draw_asteroid(surface: pygame.Surface, asteroid: Asteroid) -> None:
    ...
    """
    points = get_polygon_points(asteroid, Cfg.asteroid_vertex_count, asteroid.radius, asteroid.shape)
...
def draw_enemy(surface: pygame.Surface, enemy: Enemy) -> None:
    ...
    if is_finisher_target:
        pygame.draw.polygon(surface, Cfg.colors['gold'], points, max(1, int(3 * g_scale_factor)))
...
def draw_ship(surface: pygame.Surface, keys: dict, controller_input: Dict[str, Any]) -> None:
    ...
    if (g_ship.invulnerable > 0 and 
        g_ship.invulnerable % Cfg.ship_invulnerability_blink_interval >= Cfg.ship_invulnerability_blink_visible_frames and 
        g_ship.respawning == 0):
        return
...
def draw_respawn_animation(surface: pygame.Surface) -> None:
    ...
    if progress > Cfg.level_text_appear_threshold:
        if g_game_state['frame_count'] % Cfg.frame_visibility_check_modulo < Cfg.controller_input_check_modulo:
            return
...
def draw_powerup_flash(surface: pygame.Surface) -> None:
    ...
    max_flash = Cfg.particle_powerup_flash_max if g_ship.powerup_flash > Cfg.particle_powerup_flash_duration else Cfg.particle_powerup_flash_duration
...
def draw_ship_body(surface: pygame.Surface) -> None:
    ...
    if g_game_state['finisher']['ready']:
        pygame.draw.polygon(surface, Cfg.colors['gold'], ship_points, max(1, int(3 * g_scale_factor)))
    else:
        pygame.draw.polygon(surface, Cfg.colors['white'], ship_points, max(1, int(2 * g_scale_factor)))

def draw_thruster_flame(surface: pygame.Surface, keys: dict, controller_input: Dict[str, Any]) -> None:
    ...
    flame_length = scaled(Cfg.ship_back_indent) + random.randint(int(Cfg.flame_length_min * g_scale_factor), 
                                                                 int(Cfg.flame_length_max * g_scale_factor))
    flame_width = random.randint(int(Cfg.flame_width_min * g_scale_factor), 
                                int(Cfg.flame_width_max * g_scale_factor))
    
    flame_tip = (
        g_ship.x - flame_length * cos_a,
        g_ship.y - flame_length * sin_a
    )
    ...
    for powerup in g_powerups:
        ...
        if powerup.type == PowerUpType.CRYSTAL:
            ...
            for i in range(4):
                ...
                sin_a, cos_a = get_sin_cos(point_angle)
                points.append((powerup.x + scaled(Cfg.powerup_visual_radius) * cos_a,
                             powerup.y + scaled(Cfg.powerup_visual_radius) * sin_a))
            pygame.draw.polygon(surface, color, points, max(1, int(2 * g_scale_factor)))
        else:
            points = get_polygon_points(powerup, Cfg.powerup_hexagon_vertices, 
...
def draw_dash_preview(surface: pygame.Surface, target: Enemy, pulse: float) -> None:
    ...
    for i in range(segments):
        if i % 2 == 0:
            ...
            dim_gold = (int(Cfg.colors['gold'][0] * alpha / 255), 
...
def draw_lock_on_indicator(surface: pygame.Surface) -> None:
    ...
    """
    finisher_state = g_game_state['finisher']
    if finisher_state['phase'] != FinisherPhase.LOCK_ON or not finisher_state['target'] or finisher_state['target'] not in g_enemies:
        return
    ...
    if finisher_state['lock_on_progress'] > Cfg.finisher_lock_text_appear_threshold:
        text_alpha = int(255 * ((finisher_state['lock_on_progress'] - Cfg.finisher_lock_text_appear_threshold) * 2))
        lock_text = g_text_cache.get_text("LOCK ON", g_small_font, Cfg.colors['gold'])
        lock_rect = lock_text.get_rect(center=(int(finisher_state['target'].x), 
...
def draw_lock_on_reticle(surface: pygame.Surface, target: Enemy, progress: float) -> None:
    ...
    """
    reticle_size = int((Cfg.finisher_reticle_size_base + Cfg.finisher_reticle_size_variation * 
                       abs(math.sin(progress * math.pi))) * g_scale_factor * progress)
    if reticle_size <= 0:
        return
    ...
    for i in range(4):
        ...
        pygame.draw.line(surface, Cfg.colors['gold'],
                       (int(start_x), int(start_y)),
...
def draw_shockwave(surface: pygame.Surface, x: float, y: float, radius: float) -> None:
    ...
    for i in range(3):
        ...
        if ring_radius > 0:
            ...
            if alpha > 0:
                ring_surface = pygame.Surface((surface.get_width(), surface.get_height()), 
                                            pygame.SRCALPHA)
                
                pygame.draw.circle(ring_surface, (*Cfg.colors['gold'], alpha), 
                                 (int(x), int(y)), ring_radius, 
                ...
                if i == 0:
                    for j in range(3):
                        ...
                        if glow_alpha > 0:
                            pygame.draw.circle(ring_surface, (*Cfg.colors['gold'], glow_alpha),
                                             (int(x), int(y)), ring_radius + j * 2, 1)
                
                surface.blit(ring_surface, (0, 0))

# === [UI DRAWING] ===

def draw_ui_text(surface: pygame.Surface, text: str, x: int, y: int, 
                font_obj: Optional[pygame.font.Font] = None, color: Tuple[int, int, int] = Cfg.colors['white']) -> pygame.Surface:
...
def draw_ui(surface: pygame.Surface) -> None:
    ...
    y_offset += spacing
    draw_ui_text(surface, f"Lives: {'♥' * g_game_state['lives']}", margin, y_offset, g_font, (255, 100, 100))
    
    y_offset += spacing
    draw_ui_text(surface, f"Level: {g_game_state['level']}", margin, y_offset)
    
    y_offset += spacing
    draw_ui_text(surface, f"◆ {g_game_state['crystals']}", margin, y_offset, g_font, Cfg.colors['crystal'])
...
def draw_score_and_combo(surface: pygame.Surface, y_offset: int) -> None:
    ...
    score_text = g_text_cache.get_text(f"Score: {g_game_state['score']}", g_font, Cfg.colors['white'])
    ...
    if combo['current'] > 1:
        ...
        combo_rect = combo_text.get_rect(left=int(scaled(Cfg.ui_margin) + score_text.get_width() + 10), 
                                         top=int(y_offset))
...
def draw_combo_progress(surface: pygame.Surface, x_start: int, y_offset: int) -> None:
    ...
    for i in range(Cfg.combo_pulse_interval):
        ...
        if i < kills_in_cycle:
            pygame.draw.circle(surface, Cfg.colors['gold'], 
                             (pip_x, pip_y), 
                             int(Cfg.ui_combo_pip_radius * g_scale_factor))
        else:
            pygame.draw.circle(surface, (100, 100, 100), 
                             (pip_x, pip_y), 
                             int(Cfg.ui_combo_pip_radius * g_scale_factor), 1)
...
def draw_high_score(surface: pygame.Surface) -> None:
    ...
    """
    high_score_text = g_text_cache.get_text(f"High: {g_game_state.get('high_score', 0)}", g_font, Cfg.colors['white'])
    high_score_rect = high_score_text.get_rect(center=(g_screen_width // 2, 
...
def draw_dash_indicator(surface: pygame.Surface, y_offset: int) -> None:
    ...
    if g_game_state['dash']['cooldown'] > 0:
        ...
        pygame.draw.rect(surface, (50, 50, 50), (bar_x, bar_y, bar_width, bar_height))
        pygame.draw.rect(surface, Cfg.colors['dash'], (bar_x, bar_y, int(bar_width * cooldown_percent), bar_height))
        pygame.draw.rect(surface, Cfg.colors['white'], (bar_x, bar_y, bar_width, bar_height), 1)
        ...
        if g_game_state['finisher']['ready']:
            dash_text = g_text_cache.get_text("DASH/FINISHER READY! (SHIFT)", g_small_font, Cfg.colors['gold'])
        else:
            dash_text = g_text_cache.get_text("DASH READY (SHIFT)", g_small_font, Cfg.colors['dash'])
...
def draw_finisher_meter(surface: pygame.Surface, y_offset: int) -> None:
    ...
    pygame.draw.rect(surface, border_color, (bar_x, y_offset, meter_width, meter_height), 
                    border_thickness)
    ...
    if finisher['ready']:
        ...
        finisher_text = g_text_cache.get_text(f"FINISHER {int(finisher['meter'])}%", g_tiny_font, (150, 150, 150))
...
def draw_system_status(surface: pygame.Surface) -> None:
    ...
    sound_text = g_text_cache.get_text(f"Sound: {sound_status} (S)", g_small_font, Cfg.colors['white'])
    sound_text_rect = sound_text.get_rect()
    sound_text_rect.topright = (g_screen_width - int(scaled(Cfg.ui_margin)), int(scaled(Cfg.ui_margin)))
    ...
    controller_text = g_text_cache.get_text(f"Controller: {controller_status}", g_small_font, controller_color)
    controller_text_rect = controller_text.get_rect()
    controller_text_rect.topright = (g_screen_width - int(scaled(Cfg.ui_margin)), 
...
def draw_powerup_indicators(surface: pygame.Surface) -> None:
    ...
    if g_ship.rapid_fire > 0:
        text = g_text_cache.get_text(f"RAPID FIRE: {int(g_ship.rapid_fire // Cfg.fps)}s", 
                                  g_small_font, Cfg.powerup_types[PowerUpType.RAPID]['color'])
        surface.blit(text, (int(scaled(Cfg.ui_margin)), y_offset))
        y_offset += int(Cfg.ui_powerup_indicator_spacing * g_scale_factor)
    
    if g_ship.triple_shot > 0:
        text = g_text_cache.get_text(f"TRIPLE SHOT: {int(g_ship.triple_shot // Cfg.fps)}s", 
                                  g_small_font, Cfg.powerup_types[PowerUpType.TRIPLE]['color'])
        surface.blit(text, (int(scaled(Cfg.ui_margin)), y_offset))
        y_offset += int(Cfg.ui_powerup_indicator_spacing * g_scale_factor)
    
    if g_ship.shield_active > 0:
        text = g_text_cache.get_text(f"SHIELD: {int(g_ship.shield_active // Cfg.fps)}s", 
                                  g_small_font, Cfg.powerup_types[PowerUpType.SHIELD]['color'])
        surface.blit(text, (int(scaled(Cfg.ui_margin)), y_offset))
...
def draw_wave_warning(surface: pygame.Surface) -> None:
    ...
    if g_game_state['effects']['wave_warning'] > 0:
        ...
        warning_text = g_big_font.render(g_game_state['effects']['wave_warning_text'], True, warning_color)
...
def draw_centered_text(surface: pygame.Surface, text: str, font_obj: pygame.font.Font, 
                      color: Tuple[int, int, int], y_pos: int) -> pygame.Rect:
...
def draw_upgrade_menu(surface: pygame.Surface) -> None:
    ...
    draw_centered_text(surface, f"Crystals: ◆ {g_game_state['crystals']}", g_font, 
                      Cfg.colors['crystal'], 100 * g_scale_factor)
    
    draw_upgrade_list(surface)
    
    draw_centered_text(surface, "↑↓ Select   ENTER Purchase   ESC Close", g_font, 
                      Cfg.colors['white'], g_screen_height - 50 * g_scale_factor)
    ...
    for i, upgrade_type in enumerate(upgrade_keys):
        ...
        if i == g_game_state['selected_upgrade']:
            pygame.draw.rect(surface, (50, 50, 100), 
                           (g_screen_width // 4, y_offset - 10, g_screen_width // 2, 
                            Cfg.ui_upgrade_menu_item_height * g_scale_factor - 20))
        
        name_text = g_font.render(f"{upgrade['name']} (Lv {level}/{max_level})", True, Cfg.colors['white'])
        ...
        if level < max_level:
            ...
            cost_color = Cfg.colors['crystal'] if g_game_state['crystals'] >= cost else (255, 100, 100)
            cost_text = g_font.render(f"Cost: ◆ {cost}", True, cost_color)
        else:
            cost_text = g_font.render("MAXED", True, Cfg.colors['gold'])
        
        surface.blit(cost_text, (g_screen_width * 3 // 4 - cost_text.get_width() - Cfg.ui_upgrade_menu_padding, y_offset))
...
def draw_game_over(surface: pygame.Surface) -> None:
    ...
    """
    g_game_state['effects']['game_over_alpha'] = min(g_game_state['effects']['game_over_alpha'] + Cfg.game_over_fade_speed, 128)
    ...
    if g_game_state['effects']['game_over_alpha'] > 64:
        draw_centered_text(surface, "GAME OVER", g_big_font, (255, 0, 0), 
        ...
        y_offset += 30
        draw_centered_text(surface, "Press U to open upgrade menu", g_small_font, (200, 200, 200), y_offset)
        
        if g_controller_connected:
            y_offset += 30
            draw_centered_text(surface, "(or press START on controller)", g_small_font, (200, 200, 200), y_offset)
...
def draw_pause_overlay(surface: pygame.Surface) -> None:
    ...
    if g_game_state['effects']['pause_menu_alpha'] > 128:
        ...
        text_rect = pause_text.get_rect(center=(g_screen_width // 2, 
        ...
        achievements_text = g_small_font.render(
            ...
            True, Cfg.colors['gold'])
        achievements_text.set_alpha(calculate_fade(alpha_ratio))
        ach_rect = achievements_text.get_rect(center=(g_screen_width // 2, 
...
def apply_combo_pulse(surface: pygame.Surface) -> None:
    ...
    if g_game_state['combo']['pulse'] > Cfg.combo_edge_pulse_threshold:
        edge_alpha = min(50, (g_game_state['combo']['pulse'] - Cfg.combo_edge_pulse_threshold) * Cfg.combo_edge_pulse_multiplier)
        for i in range(3):
            edge_surface = pygame.Surface((g_screen_width, g_screen_height), pygame.SRCALPHA)
            thickness = int((Cfg.combo_edge_thickness_base - i * Cfg.combo_edge_thickness_step) * g_scale_factor)
            ...
            pygame.draw.rect(edge_surface, color, (0, g_screen_height - thickness, g_screen_width, thickness))
            pygame.draw.rect(edge_surface, color, (0, 0, thickness, g_screen_height))
            pygame.draw.rect(edge_surface, color, (g_screen_width - thickness, 0, thickness, g_screen_height))
...
def render_background(surface: pygame.Surface, shake_x: int, shake_y: int) -> None:
    ...
    vfx.draw_stars(surface, shake_x, shake_y, g_game_state['frame_count'], g_screen_width, g_screen_height)
...
def render_game_objects(game_surface: pygame.Surface, keys: dict, controller_input: Dict[str, Any]) -> None:
...
def render_effects_and_ui(surface: pygame.Surface, game_surface: pygame.Surface, 
                         shake_x: int, shake_y: int) -> None:
    ...
    vfx.draw_level_transition(surface, g_game_state, g_screen_width, g_screen_height, g_scale_factor)
...
def render_to_surface(surface: pygame.Surface, shake_x: int, shake_y: int, 
                     keys: dict, controller_input: Dict[str, Any]) -> None:
...
def reset_game() -> None:
    ...
    persistent_data = {
        ...
        'upgrade_levels': g_game_state.get('upgrade_levels', g_progression.get_default_upgrade_levels()),
        'boss_kills': g_game_state.get('boss_kills', 0)
    }
...
def handle_events() -> bool:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.WINDOWFOCUSLOST and not g_game_state['game_over'] and not g_game_state['paused']:
...
def update_game_state(keys: dict, controller_input: Dict[str, Any]) -> None:
    ...
    if g_game_state['effects']['level_transition'] > 0:
        g_game_state['effects']['level_transition'] = update_timer(g_game_state['effects']['level_transition'])
        if g_game_state['effects']['level_transition'] == 0:
            start_new_level()
    
    if g_game_state['effects']['damage_flash'] > 0:
        g_game_state['effects']['damage_flash'] = update_timer(g_game_state['effects']['damage_flash'], 2)
...
def run_game_loop() -> None:
    ...
    while running:
        ...
        if g_game_state['resize_pending'] and pygame.time.get_ticks() - g_game_state['resize_timer'] > Cfg.resize_debounce_time:
            handle_resize(*g_game_state['resize_pending'])
            g_game_state['resize_pending'] = None
        
        if not g_game_state['game_over'] and not g_game_state['paused'] and not g_game_state['show_upgrade_menu']:
            current_dash = keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT] or controller_input['dash']
            prev_keys_dash, prev_controller_dash = handle_dash_input(
                current_dash, prev_keys_dash, prev_controller_dash)
            
            if controller_input['shoot'] and not prev_controller_shoot and g_game_state['bullet_cooldown'] <= 0 and g_ship.respawning == 0:
                shoot_bullet()
...
def handle_upgrade_menu_keys(event: pygame.event.Event) -> None:
    ...
    if event.key == pygame.K_UP:
        g_game_state['selected_upgrade'] = (g_game_state['selected_upgrade'] - 1) % len(Cfg.upgrades)
...
def handle_game_keys(event: pygame.event.Event) -> None:
    ...
    Globals:
        Reads/writes g_game_state, g_ship
    """
    if event.key == pygame.K_SPACE and not g_game_state['game_over'] and g_game_state['bullet_cooldown'] <= 0 and g_ship.respawning == 0:
        shoot_bullet()
...
def handle_dash_input(current_dash: bool, prev_keys_dash: bool, 
                     prev_controller_dash: bool) -> Tuple[bool, bool]:
    ...
    if current_dash and not prev_keys_dash and not prev_controller_dash:
        if g_game_state['dash']['cooldown'] <= 0 and g_ship.respawning == 0 and g_ship.dashing == 0:
            if g_game_state['finisher']['ready'] and not g_game_state['finisher']['executing'] and g_enemies:
                target = check_finisher_collision(g_ship, g_ship.angle)
                if target:
...
def update_pause_menu_fade() -> None:
    ...
    if g_game_state['effects']['pause_menu_alpha'] < 255 and g_game_state['paused']:
        g_game_state['effects']['pause_menu_alpha'] = min(255, g_game_state['effects']['pause_menu_alpha'] + Cfg.pause_fade_speed)
    elif g_game_state['effects']['pause_menu_alpha'] > 0 and not g_game_state['paused']:
        g_game_state['effects']['pause_menu_alpha'] = max(0, g_game_state['effects']['pause_menu_alpha'] - Cfg.pause_fade_speed)
...
def calculate_screen_shake() -> Tuple[int, int]:
    ...
    if g_game_state['effects']['screen_shake'] > 0:
        if not g_game_state['paused']:
            g_game_state['effects']['screen_shake'] = max(0, g_game_state['effects']['screen_shake'] - Cfg.screen_shake_decay)
        shake_x = random.randint(-int(g_game_state['effects']['screen_shake']), int(g_game_state['effects']['screen_shake']))
        shake_y = random.randint(-int(g_game_state['effects']['screen_shake']), int(g_game_state['effects']['screen_shake']))
