...
class DrawingSystem:
    ...
    def glow(
        ...
        for i in range(layers):
            ...
            pygame.draw.circle(
                glow_surface, (*color, alpha), (scaled_radius, scaled_radius), layer_radius
            )
    ...
    def polygon_with_flash(
        ...
        if entity.hit_flash > 0:
            ...
            pygame.draw.polygon(
                surface, base_color, points, max(1, int(border_width * scale_factor))
            )
    ...
    def health_bar(
        ...
        pygame.draw.rect(
            surface, health_color, (bar_x, bar_y, bar_width * health_percent, bar_height)
        )
    ...
    def enemy_health_pips(self, surface: pygame.Surface, enemy: Enemy) -> None:
        ...
        for i in range(int(enemy.health)):
            pip_x = (
                enemy.x
                - Cfg.ui_health_pip_spacing * scale_factor
                + i * Cfg.ui_health_pip_spacing * scale_factor
            )
            pip_y = enemy.y + 20 * scale_factor
            pygame.draw.circle(
                surface, (255, 100, 100), (int(pip_x), int(pip_y)), int(2 * scale_factor)
            )
    ...
    def _calculate_pulse(
        self, time: float, frequency: float = 0.1, amplitude: float = 0.5, offset: float = 0.5
    ) -> float:
    ...
    def _get_polygon_points(
        ...
        angle_start = (
            ...
            else (obj.angle if hasattr(obj, "angle") else 0)
        )
    ...
    def _get_ship_points(
        self, x: float, y: float, angle: float, scale_mult: float = 1.0
    ) -> List[Tuple[float, float]]:
    ...
    def draw_asteroid(
        self, surface: pygame.Surface, asteroid: Asteroid, get_polygon_points_func
    ) -> None:
        ...
        """
        points = get_polygon_points_func(
        ...
        self.glow(surface, (asteroid.x, asteroid.y), asteroid.size * 15 * scale_factor, glow_color)
    ...
    def draw_enemy(
        ...
        if is_finisher_target:
            ...
            self.glow(
                surface, (enemy.x, enemy.y), 25 * pulse * scale_factor, Cfg.colors["gold"], pulse
            )
        else:
            self.glow(
                surface,
                (enemy.x, enemy.y),
                enemy.radius * 1.2 * scale_factor,
                Cfg.colors["enemy"],
                0.8,
            )
        ...
        if enemy.ai_type == EnemyAIType.HUNTER:
            ...
            pygame.draw.circle(
                surface, (200, 50, 50), (int(enemy.x), int(enemy.y)), int(3 * scale_factor), 1
            )
    ...
    def draw_ship(
        ...
        if (
            ship.invulnerable > 0
            and ship.invulnerable % Cfg.ship_invulnerability_blink_interval
            >= Cfg.ship_invulnerability_blink_visible_frames
            and ship.respawning == 0
        ):
            return
    ...
    def draw_respawn_animation(
        self, surface: pygame.Surface, ship: ShipState, game_state: Dict[str, Any]
    ) -> None:
        ...
        for i in range(Cfg.respawn_spiral_layers):
            ...
            if radius > 0:
                ...
                self.glow(surface, (ship.x, ship.y), radius, Cfg.colors["blue_glow"], alpha)

        if progress > Cfg.level_text_appear_threshold:
            if (
                game_state["frame_count"] % Cfg.frame_visibility_check_modulo
    ...
    def draw_powerup_flash(self, surface: pygame.Surface, ship: ShipState) -> None:
        ...
        max_flash = (
            ...
            if ship.powerup_flash > Cfg.particle_powerup_flash_duration
        ...
        if flash_intensity > 0:
            ...
            self.glow(surface, (ship.x, ship.y), radius, ship.powerup_flash_color, flash_intensity)
    ...
    def draw_ship_body(
        ...
        if game_state["finisher"]["ready"]:
            pygame.draw.polygon(
                surface, Cfg.colors["gold"], ship_points, max(1, int(3 * scale_factor))
            )
        else:
            pygame.draw.polygon(
                surface, Cfg.colors["white"], ship_points, max(1, int(2 * scale_factor))
            )
    ...
    def draw_thruster_flame(
        ...
        flame_length = self._scaled(Cfg.ship_back_indent) + random.randint(
            int(Cfg.flame_length_min * scale_factor), int(Cfg.flame_length_max * scale_factor)
        )
        flame_width = random.randint(
            int(Cfg.flame_width_min * scale_factor), int(Cfg.flame_width_max * scale_factor)
        )

        flame_tip = (ship.x - flame_length * cos_a, ship.y - flame_length * sin_a)
    ...
    def draw_powerups(self, surface: pygame.Surface, powerups: List[PowerUp]) -> None:
        ...
        for powerup in powerups:
            ...
            self.glow(surface, (powerup.x, powerup.y), 25 * pulse * scale_factor, color, pulse)

            symbol_text = self.fonts["small"].render(
            ...
            if powerup.type == PowerUpType.CRYSTAL:
                ...
                for i in range(4):
                    ...
                    points.append(
                        (
                            powerup.x + self._scaled(Cfg.powerup_visual_radius) * cos_a,
    ...
    def draw_bullets(
        self, surface: pygame.Surface, bullets: List[Any], enemy_bullets: List[Any]
    ) -> None:
        """Draw all bullets.
        ...
        for bullet in bullets:
            ...
            self.glow(
                surface, (bullet.x, bullet.y), self._scaled(Cfg.bullet_radius * 4), (255, 255, 0)
            )
            pygame.draw.circle(
                surface, Cfg.colors["bullet"], (int(bullet.x), int(bullet.y)), int(3 * scale_factor)
            )
        ...
        for bullet in enemy_bullets:
            ...
            self.glow(
                surface,
                (bullet.x, bullet.y),
                self._scaled(Cfg.bullet_radius * 4) * 0.8,
                (255, 100, 100),
                0.8,
            )
            pygame.draw.circle(
        ...
        for particle in particle_pool.get_active_particles():
            ...
            if "has_glow" in renderer:
                ...
                if has_glow:
                    ...
                    glow_color_list = [
                        max(0, min(255, int(c * life_ratio * glow_factor))) for c in particle.color
                    ]
                    glow_color = (glow_color_list[0], glow_color_list[1], glow_color_list[2])
                    self.glow(
                        surface,
