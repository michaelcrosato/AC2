...
class ProgressionSystem:
    ...
    def __init__(self, create_floating_text: Callable, update_scaled_values: Callable):
        ...
        self.achievement_conditions = {
            ...
            "combo_5": lambda ctx: ctx.game_state["combo"]["current"]
            >= Cfg.combo_milestone_thresholds[0],
            "combo_10": lambda ctx: ctx.game_state["combo"]["current"]
            ...
            "untouchable": lambda ctx: ctx.game_state["level"] > 1
            and ctx.game_state["untouchable_level"],
            "speed_demon": lambda ctx: ctx.game_state["upgrade_levels"]["max_speed"]
            >= Cfg.achievement_speed_demon_level,
            "crystal_hoarder": lambda ctx: ctx.game_state["lifetime_crystals"]
    ...
    def load_game_state(self, ctx: GameContext) -> bool:
        ...
        try:
            ...
            print(
                f"[load_game_state] Loaded: High score {ctx.game_state['high_score']}, {achievement_count} achievements"
            )
            return True
