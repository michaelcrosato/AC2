...
def draw_stars(
    ...
    for star in g_stars:
        ...
        if star["size"] == 1:
            ...
            pygame.draw.circle(
                surface, glow_color, (x % screen_width, y % screen_height), glow_radius
            )
...
def update_and_draw_dust(
    surface: pygame.Surface, ctx: Dict[str, Any], offset_x: int = 0, offset_y: int = 0
) -> None:
...
def draw_crt_effects(
    surface: pygame.Surface, screen_width: int, screen_height: int, scale_factor: float
) -> None:
...
def draw_level_transition(
    ...
    if progress > Cfg.level_text_appear_threshold:
        draw_level_transition_text(
            surface, progress, game_state, screen_width, screen_height, scale_factor
        )


def draw_level_transition_text(
    surface: pygame.Surface,
    progress: float,
    game_state: Dict[str, Any],
    screen_width: int,
    ...
    text_progress = (progress - Cfg.level_text_appear_threshold) / (
        1 - Cfg.level_text_appear_threshold
    )

    if text_progress < Cfg.level_text_fade_threshold:
        scale = (
            Cfg.level_text_scale_min
            + (Cfg.level_text_scale_max - Cfg.level_text_scale_min) * text_progress
        )
        alpha = 255
    else:
        scale = Cfg.level_text_scale_max
        fade_progress = (text_progress - Cfg.level_text_fade_threshold) / (
            1 - Cfg.level_text_fade_threshold
        )
...
def draw_damage_flash(
    ...
    max_duration = (
        ...
        if game_state["effects"]["damage_flash_color"][0] > 200
    ...
    if game_state["effects"]["damage_flash"] > max_duration * Cfg.damage_flash_pulse_threshold:
        pulse = (
            math.sin(
                (max_duration - game_state["effects"]["damage_flash"])
    ...
    for i in range(max_layers):
        ...
        pygame.draw.rect(
            vignette, color, (0, screen_height - inset - thickness, screen_width, thickness)
        )
        pygame.draw.rect(vignette, color, (inset, inset, thickness, screen_height - inset * 2))
        pygame.draw.rect(
            vignette,
            color,
            (screen_width - inset - thickness, inset, thickness, screen_height - inset * 2),
        )
